...
from .views import UserProfileView, UserProfileImageUploadView, UserProfileUpdateView, ChangePasswordView, Toggle2FAView
...
urlpatterns = [
    ...
    # User profile endpoints for dashboard
    path('users/me/', UserProfileView.as_view(), name='user-profile'),  # GET, PUT
    path('users/me/update/', UserProfileUpdateView.as_view(), name='user-profile-update'),  # PUT, PATCH (alias)
    path('users/me/profile-image/', UserProfileImageUploadView.as_view(), name='user-profile-image-upload'),  # PUT only (image)
    
    # Security settings endpoints
    path('auth/change-password/', ChangePasswordView.as_view(), name='change-password'),  # POST
    path('auth/toggle-2fa/', Toggle2FAView.as_view(), name='toggle-2fa'),  # POST
]
