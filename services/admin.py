...
class ServiceCategoryAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Basic Information', {
            'fields': ('name', 'slug', 'short_desc', 'category_url')
        }),
        ('Subcategories', {
            'fields': ('sub_categories',),
            'description': 'Enter subcategories as JSON array. Example: [{"id": "web-design", "name": "Web Design"}, {"id": "e-commerce", "name": "E-commerce"}]'
        }),
        ('Settings', {
    ...
    def short_desc_preview(self, obj):
        """Show truncated short description"""
        if obj.short_desc:
            return (obj.short_desc[:50] + '...') if len(obj.short_desc) > 50 else obj.short_desc
        return '-'
    short_desc_preview.short_description = 'Description'
    
    def category_image_preview(self, obj):
        """Show small preview of category image"""
        if obj.category_url:
            return format_html(
                '<img src="{}" style="width: 30px; height: 30px; object-fit: cover; border-radius: 4px;" />',
                obj.category_url.url
            )
        return '-'
...
class ServiceAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Basic Information', {
            'fields': ('name', 'slug', 'service_category')
        }),
        ('Content', {
...
class ServiceProcessStepAdmin(admin.ModelAdmin):
    ...
    list_filter = ['service__service_category']
