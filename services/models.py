...
def validate_image_size(image):
    """Validate that image file size is under 3MB"""
    if image:
        max_size = 3 * 1024 * 1024  # 3MB in bytes
        if image.size > max_size:
            raise ValidationError(
                f'Image file size must be under 3MB. Current size: {image.size / (1024 * 1024):.2f}MB'
            )
...
# This is what we are current extending
class ServiceCategory(models.Model):
    """
    Service categories with optional subcategories
    """
    ...
    category_url = models.ImageField(
    ...
    def get_subcategory_names(self):
        ...
        if not self.sub_categories:
            return []
        return [sub.get('name', '') for sub in self.sub_categories]
