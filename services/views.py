...
from .models import (
    Service, ServiceCategory, ServicePricingTier, ServiceFeature, ServiceProcessStep,
...
class ServiceCategoryViewSet(viewsets.ModelViewSet):
    ...
    Provides CRUD operations for service categories with search and filtering capabilities.
    """
    ...
    ordering_fields = ['name', 'date_created', 'date_updated', 'sort_order']
    ordering = ['sort_order', 'name']
    
    def get_serializer_class(self):
        """
        Return appropriate serializer based on action.
        Use detailed serializer for list and retrieve actions to include services.
        """
        if self.action in ['list', 'retrieve']:
            return ServiceCategoryWithServicesSerializer
        return ServiceCategorySerializer

    def get_queryset(self):
        """
        Return service categories based on user permissions.
        Authenticated users can see all categories, anonymous users see only active ones.
        """
    ...
    def services(self, request, pk=None):
        ...
        if request.user.is_authenticated:
            ...
            serializer = PublicServiceListSerializer(services, many=True, context={'request': request})
    ...
    def stats(self, request):
        """
        Get statistics about service categories.
        """
        queryset = self.get_queryset()
        stats = {
            'total_categories': queryset.count(),
            'active_categories': queryset.filter(active=True).count(),
            'categories_with_services': queryset.filter(services__isnull=False).distinct().count(),
        }
...
class ServiceViewSet(viewsets.ModelViewSet):
    ...
    lookup_field = 'slug'
    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    filterset_fields = ['service_category', 'pricing_model', 'featured', 'active']
    ...
    def by_category(self, request):
        ...
        services = self.get_queryset().filter(service_category__name__iexact=category)
    ...
    def pricing_overview(self, request):
        ...
        # Group by category
        categories = services.values('service_category__name').annotate(
            count=Count('id'),
            avg_price=Avg('starting_at')
        ).order_by('service_category__name')
...
class ServicesByCategoryAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return Service.objects.filter(
            service_category__name__iexact=category,
            active=True
