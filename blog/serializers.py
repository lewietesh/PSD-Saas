...
class BlogCommentCreateSerializer(serializers.ModelSerializer):
    ...
    blogpost = serializers.CharField(write_only=True)

    class Meta:
        model = BlogComment
        fields = ['blogpost', 'name', 'email', 'website', 'message', 'parent']

    def validate_blogpost(self, value):
        from .models import BlogPost
        try:
            post = BlogPost.objects.get(id=value)
        except BlogPost.DoesNotExist:
            raise serializers.ValidationError("No BlogPost matches the given ID.")
        if post.status != 'published':
            raise serializers.ValidationError("Comments are only allowed on published posts.")
        return value

    def create(self, validated_data):
        blogpost_id = validated_data.pop('blogpost')
        from .models import BlogPost
        blogpost = BlogPost.objects.get(id=blogpost_id)
        return BlogComment.objects.create(blogpost=blogpost, **validated_data)
...
class BlogPostCreateUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate_excerpt(self, value):
        ...
        if len(value.strip()) > 1500:
            raise serializers.ValidationError("Excerpt cannot exceed 1500 characters.")
        return value.strip()
