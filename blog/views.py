...
class BlogCommentViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        """Allow public comment creation with input sanitization and no file uploads"""
        from django.utils.html import strip_tags
        data = request.data.copy()
        # Only allow safe fields
        allowed_fields = {'blogpost', 'name', 'email', 'website', 'message', 'parent'}
        for key in list(data.keys()):
            if key not in allowed_fields:
                data.pop(key)
        # Sanitize string fields
        for field in ['name', 'email', 'website', 'message']:
            if field in data and isinstance(data[field], str):
                data[field] = strip_tags(data[field])
        # Block file uploads
        if any(hasattr(f, 'file') for f in request.FILES.values()):
            return Response({'detail': 'File uploads are not allowed.'}, status=status.HTTP_400_BAD_REQUEST)
        serializer = BlogCommentCreateSerializer(data=data)
        if serializer.is_valid():
            comment = serializer.save(approved=False)  # Always require moderation
